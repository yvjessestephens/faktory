...
func NewPool(capacity int) (*Pool, error) {
	// println("yv faktory/client/pool.go > NewPool: entry") // jesses debug
...
func newPool(capacity int, opener pool.Factory) (*Pool, error) {
	// println("yv faktory/internal/pool/pool.go > newPool: entry") // jesses debug
	var p Pool
	var err error
...
func (p *Pool) Get() (*Client, error) {
	// println("yv faktory/internal/pool/pool.go > Get: entry") // jesses debug
	conn, err := p.Pool.Get()
	if err != nil {
		return nil, err
	}
